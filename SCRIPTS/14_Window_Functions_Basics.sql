...
sum(sales) over(partition by orderstatus order by orderdate rows between 2 preceding and current row)Total_sales from orders;

/* TASK 10: 
   Calculate cumulative Total Sales by Order Status up to the current order 
*/
select orderid,orderdate,productid,orderstatus,
sum(sales) over(partition by orderstatus order by orderdate rows between unbounded preceding and current row) cumulative_total_sales
from orders;
...
*/
select orderid,orderdate,productid,orderstatus,
sum(sales) over(partition by orderstatus order by orderdate rows unbounded preceding) total_sales_cumulative
from orders;
-- short form of window frame function 
